...
public class StreamState {
    ...
    public int packetout(Packet op) {
        ...
        if ((this.lacing_vals[ptr] & 1024) != 0) {
            ++this.packetno;
            return -1;
        }
    ...
    public int pagein(Page og) {
        ...
        if (_pageno != this.pageno) {
            for (int i = this.lacing_packet; i < this.lacing_fill; ++i) {
                this.body_fill -= this.lacing_vals[i] & 255;
            }
        ...
        int saved = -1;
        while (segptr < segments) {
            this.lacing_vals[this.lacing_fill] = val = header_base[header + 27 + segptr] & 255;
    ...
    public int flush(Page og) {
        ...
        bytes = 0;
        int acc = 0;
        granule_pos = this.granule_vals[0];
        if (maxvals == 0) {
            return 0;
        }
        if (this.b_o_s == 0) {
            ...
            for (vals = 0; vals < maxvals; ++vals) {
                if ((this.lacing_vals[vals] & 255) >= 255) continue;
                ++vals;
                break;
            }
        } else {
            for (vals = 0; vals < maxvals && acc <= 4096; acc += this.lacing_vals[vals] & 255, ++vals) {
        ...
        if ((this.lacing_vals[0] & 256) == 0) {
            byte[] arrby = this.header;
            arrby[5] = (byte)(arrby[5] | 1);
        }
        if (this.b_o_s == 0) {
            byte[] arrby = this.header;
            arrby[5] = (byte)(arrby[5] | 2);
        }
        if (this.e_o_s != 0 && this.lacing_fill == vals) {
            byte[] arrby = this.header;
            arrby[5] = (byte)(arrby[5] | 4);
        }
        this.b_o_s = 1;
        for (i = 6; i < 14; ++i) {
