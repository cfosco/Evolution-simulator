...
extends BaseShader {
    ...
    public DefaultShader(Renderable renderable, Config config, ShaderProgram shaderProgram) {
        ...
        this.environmentCubemap = renderable.material.has(CubemapAttribute.EnvironmentMap) || this.lighting && renderable.environment.has(CubemapAttribute.EnvironmentMap);
        this.shadowMap = this.lighting && renderable.environment.shadowMap != null;
        this.fog = this.lighting && renderable.environment.has(ColorAttribute.Fog);
    ...
    public static String createPrefix(Renderable renderable, Config config) {
        ...
        if (DefaultShader.and(attributes, 1)) {
            prefix = prefix + "#define positionFlag\n";
        }
        if (DefaultShader.or(attributes, 6)) {
            prefix = prefix + "#define colorFlag\n";
        }
        if (DefaultShader.and(attributes, 256)) {
            prefix = prefix + "#define binormalFlag\n";
        }
        if (DefaultShader.and(attributes, 128)) {
            prefix = prefix + "#define tangentFlag\n";
        }
        if (DefaultShader.and(attributes, 8)) {
            prefix = prefix + "#define normalFlag\n";
        }
        if ((DefaultShader.and(attributes, 8) || DefaultShader.and(attributes, 384)) && renderable.environment != null) {
    ...
    public boolean canRender(Renderable renderable) {
        return this.materialMask == (renderable.material.getMask() | optionalAttributes) && this.vertexMask == renderable.mesh.getVertexAttributes().getMask() && renderable.environment != null == this.lighting && (renderable.environment != null && renderable.environment.has(ColorAttribute.Fog)) == this.fog;
    }
    ...
    protected void bindLights(Renderable renderable) {
        ...
        if (lights.has(ColorAttribute.Fog)) {
            this.set(this.u_fogColor, ((ColorAttribute)lights.get((long)ColorAttribute.Fog)).color);
        }
            ...
            public void set(BaseShader shader, int inputID, Renderable renderable, Attributes combinedAttributes) {
                if (renderable.environment == null) {
                    ...
                    if (renderable.environment.has(ColorAttribute.AmbientLight)) {
                        this.cacheAmbientCubemap.set(((ColorAttribute)renderable.environment.get((long)ColorAttribute.AmbientLight)).color);
                    }
                    for (i = this.dirLightsOffset; i < renderable.environment.directionalLights.size; ++i) {
                        this.cacheAmbientCubemap.add(renderable.environment.directionalLights.get((int)i).color, renderable.environment.directionalLights.get((int)i).direction);
                    }
                    for (i = this.pointLightsOffset; i < renderable.environment.pointLights.size; ++i) {
                        this.cacheAmbientCubemap.add(renderable.environment.pointLights.get((int)i).color, renderable.environment.pointLights.get((int)i).position, tmpV1, renderable.environment.pointLights.get((int)i).intensity);
                    }
