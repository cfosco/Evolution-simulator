...
extends WidgetGroup {
    ...
    public Cell<Label> add(String text) {
        ...
        return this.add((T)new Label((CharSequence)text, this.skin));
    }
    ...
    public Cell<Label> add(String text, String labelStyleName) {
        ...
        return this.add((T)new Label((CharSequence)text, this.skin.get(labelStyleName, Label.LabelStyle.class)));
    }
    ...
    public Cell<Label> add(String text, String fontName, Color color) {
        ...
        return this.add((T)new Label((CharSequence)text, new Label.LabelStyle(this.skin.getFont(fontName), color)));
    }
    ...
    public Cell<Label> add(String text, String fontName, String colorName) {
        ...
        return this.add((T)new Label((CharSequence)text, new Label.LabelStyle(this.skin.getFont(fontName), this.skin.getColor(colorName))));
    }
    ...
    private void computeSize() {
        ...
        for (i = 0; i < cellCount; ++i) {
            ...
            if (colspan == 1 && c.expandX != 0 && expandWidth[column2] == 0.0f) {
                expandWidth[column2] = c.expandX.intValue();
            }
            c.computedPadLeft = c.padLeft.get(a) + (column2 == 0 ? 0.0f : Math.max(0.0f, c.spaceLeft.get(a) - spaceRightLast));
            ...
            c.computedPadRight = c.padRight.get(a) + (column2 + colspan == columns ? 0.0f : spaceRight);
            ...
            if (colspan == 1) {
                ...
                columnPrefWidth[column2] = Math.max(columnPrefWidth[column2], prefWidth + hpadding2);
                columnMinWidth[column2] = Math.max(columnMinWidth[column2], minWidth + hpadding2);
            }
            float vpadding2 = c.computedPadTop + c.computedPadBottom;
            rowPrefHeight[row] = Math.max(rowPrefHeight[row], prefHeight + vpadding2);
            rowMinHeight[row] = Math.max(rowMinHeight[row], minHeight + vpadding2);
        }
        block1 : for (i = 0; i < cellCount; ++i) {
            int ii;
            c = cells.get(i);
            ...
            for (ii = column; ii < nn; ++ii) {
                if (expandWidth[ii] != 0.0f) continue block1;
            }
            for (ii = column; ii < nn; ++ii) {
        ...
        for (i = 0; i < cellCount; ++i) {
            ...
            int nn3 = ii3 + colspan;
            while (ii3 < nn3) {
                float ratio = totalExpandWidth == 0.0f ? 1.0f / (float)colspan : expandWidth[ii3] / totalExpandWidth;
        ...
        for (i2 = 0; i2 < cellCount; ++i2) {
            ...
            if (c2.uniformX == Boolean.TRUE && c2.colspan == 1) {
                ...
                uniformMinWidth = Math.max(uniformMinWidth, columnMinWidth[c2.column] - hpadding);
                uniformPrefWidth = Math.max(uniformPrefWidth, columnPrefWidth[c2.column] - hpadding);
            }
        ...
        float hpadding3 = this.padLeft.get(this) + this.padRight.get(this);
        float vpadding3 = this.padTop.get(this) + this.padBottom.get(this);
    ...
    private void layout(float layoutX, float layoutY, float layoutWidth, float layoutHeight) {
        ...
        for (i5 = 0; i5 < columns; ++i5) {
            totalExpandWidth += expandWidth[i5];
        }
        for (i5 = 0; i5 < rows; ++i5) {
            totalExpandHeight += expandHeight[i5];
        }
        ...
        if (totalExpandWidth > 0.0f) {
            ...
            for (i3 = 0; i3 < columns; ++i3) {
                extra -= columnWidth[i3];
            }
            float used = 0.0f;
            int lastIndex = 0;
            for (i4 = 0; i4 < columns; ++i4) {
                if (expandWidth[i4] == 0.0f) continue;
                amount = extra * expandWidth[i4] / totalExpandWidth;
        ...
        if (totalExpandHeight > 0.0f) {
            ...
            int lastIndex = 0;
            for (i4 = 0; i4 < rows; ++i4) {
                if (expandHeight[i4] == 0.0f) continue;
                amount = extra * expandHeight[i4] / totalExpandHeight;
