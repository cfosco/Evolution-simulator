...
implements Disposable {
    ...
    public Drawable getDrawable(String name) {
        ...
        drawable = this.optional(name, TiledDrawable.class);
        if (drawable != null) {
            return drawable;
        }
        try {
    ...
    public Drawable newDrawable(Drawable drawable, Color tint) {
        BaseDrawable newDrawable2;
        BaseDrawable newDrawable2;
        if (drawable instanceof TextureRegionDrawable) {
            newDrawable2 = ((TextureRegionDrawable)drawable).tint(tint);
        } else if (drawable instanceof NinePatchDrawable) {
            newDrawable2 = ((NinePatchDrawable)drawable).tint(tint);
        } else if (drawable instanceof SpriteDrawable) {
            newDrawable2 = ((SpriteDrawable)drawable).tint(tint);
        } else {
            throw new GdxRuntimeException("Unable to copy, unknown drawable type: " + drawable.getClass());
        }
        if (newDrawable2 instanceof BaseDrawable) {
            BaseDrawable named = newDrawable2;
            if (drawable instanceof BaseDrawable) {
        ...
        json.setSerializer(BitmapFont.class, new Json.ReadOnlySerializer<BitmapFont>(){
            ...
            public BitmapFont read(Json json, JsonValue jsonData, Class type) {
                ...
                try {
                    ...
                    BitmapFont font = region != null ? new BitmapFont(fontFile, region, (boolean)flip) : ((imageFile = fontFile.parent().child(regionName + ".png")).exists() ? new BitmapFont(fontFile, imageFile, (boolean)flip) : new BitmapFont(fontFile, flip));
                    if (scaledSize != -1) {
