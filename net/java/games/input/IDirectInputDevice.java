...
final class IDirectInputDevice {
    ...
    public IDirectInputDevice(DummyWindow window, long address, byte[] instance_guid, byte[] product_guid, int dev_type, int dev_subtype, String instance_name, String product_name) throws IOException {
        ...
        for (int i = 0; i < this.objects.size(); ++i) {
            ...
            if (!obj.isAxis()) continue;
            has_axis = true;
            if (obj.isRelative()) continue;
            all_relative = false;
            break;
        }
    ...
    private final List createRumblers() throws IOException {
        ...
        long[] directions = new long[]{0};
        for (int i = 0; i < this.effects.size(); ++i) {
            DIEffectInfo info = (DIEffectInfo)this.effects.get(i);
            if ((info.getEffectType() & 255) != 3 || (info.getDynamicParams() & 4) == 0) continue;
    ...
    private final Rumbler createPeriodicRumbler(DIDeviceObject[] axes, long[] directions, DIEffectInfo info) throws IOException {
        ...
        long effect_address = IDirectInputDevice.nCreatePeriodicEffect(this.address, info.getGUID(), 17, -1, 0, 10000, -1, 0, axis_ids, directions, 0, 0, 0, 0, 10000, 0, 0, 50000, 0);
    ...
    private final DIDeviceObject lookupObjectByGUID(int guid_id) {
        for (int i = 0; i < this.objects.size(); ++i) {
            ...
            if (guid_id != object.getGUIDType()) continue;
            return object;
        }
        return null;
    }
    ...
    private final void poll() throws IOException {
        int res = IDirectInputDevice.nPoll(this.address);
        if (res != 0 && res != 1) {
            if (res == -2147024868) {
                this.acquire();
                return;
    ...
    private final void acquire() throws IOException {
        int res = IDirectInputDevice.nAcquire(this.address);
    ...
    private final void unacquire() throws IOException {
        int res = IDirectInputDevice.nUnacquire(this.address);
        if (res != 0 && res != 1) {
    ...
    private final boolean getDeviceData(DataQueue queue) throws IOException {
        int res = IDirectInputDevice.nGetDeviceData(this.address, 0, queue, queue.getElements(), queue.position(), queue.remaining());
        if (res != 0 && res != 1) {
            if (res == -2147024868) {
                this.acquire();
                return false;
    ...
    private final void getDeviceState(int[] device_state) throws IOException {
        int res = IDirectInputDevice.nGetDeviceState(this.address, device_state);
        if (res != 0) {
            if (res == -2147024868) {
    ...
    private final void setDataFormat(int flags) throws IOException {
        ...
        int res = IDirectInputDevice.nSetDataFormat(this.address, flags, device_objects);
        if (res != 0) {
    ...
    public final synchronized long[] getRangeProperty(int object_identifier) throws IOException {
        ...
        int res = IDirectInputDevice.nGetRangeProperty(this.address, object_identifier, range);
        if (res != 0) {
    ...
    public final synchronized int getDeadzoneProperty(int object_identifier) throws IOException {
        this.checkReleased();
        return IDirectInputDevice.nGetDeadzoneProperty(this.address, object_identifier);
    }
    ...
    private final Component.Identifier getIdentifier(int guid_type, int type, int instance) {
        switch (guid_type) {
            ...
            case 9: {
                return IDirectInputDevice.getKeyIdentifier(instance);
            }
            case 8: {
    ...
    public final synchronized void setBufferSize(int size) throws IOException {
        this.checkReleased();
        this.unacquire();
        int res = IDirectInputDevice.nSetBufferSize(this.address, size);
        if (res != 0 && res != 1 && res != 2) {
            throw new IOException("Failed to set buffer size (" + Integer.toHexString(res) + ")");
        }
        Class class_ = DIDeviceObjectData.class;
        this.queue = new DataQueue(size, class_);
    ...
    public final synchronized void setCooperativeLevel(int flags) throws IOException {
        this.checkReleased();
        int res = IDirectInputDevice.nSetCooperativeLevel(this.address, this.window.getHwnd(), flags);
        if (res != 0) {
