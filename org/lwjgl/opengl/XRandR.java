...
public class XRandR {
        ...
        try {
            ...
            if (primaryScreenIdentifier == null) {
                ...
                for (Screen screen : current) {
                    if (1 * (long)screen.width * (long)screen.height <= totalPixels) continue;
    ...
    public static /* varargs */ void setConfiguration(boolean disableOthers, Screen ... screens) {
        if (screens.length == 0) {
    ...
    static /* varargs */ DisplayMode ScreentoDisplayMode(Screen ... screens) {
        XRandR.populate();
        Screen primary = XRandR.findPrimary(screens);
        return new DisplayMode(primary.width, primary.height, 24, primary.freq);
    }

    private static /* varargs */ Screen findPrimary(Screen ... screens) {
        for (Screen screen : screens) {
            if (!screen.name.equals(primaryScreenIdentifier)) continue;
            return screen;
        }
