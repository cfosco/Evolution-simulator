...
final class WindowsKeyboard {
    ...
    void poll(ByteBuffer keyDownBuffer) {
        if (this.isKeyDown(42) && this.isKeyDown(54)) {
            if (!WindowsKeyboard.isKeyPressedAsync(160)) {
                this.handleKey(16, 42, false, 0, 0, false);
            }
            if (!WindowsKeyboard.isKeyPressedAsync(161)) {
                this.handleKey(16, 54, false, 0, 0, false);
            }
        }
    ...
    private void putEvent(int keycode, byte state, int ch, long millis, boolean repeat) {
        ...
        this.tmp_event.putInt(keycode).put(state).putInt(ch).putLong(millis * 1000000).put(repeat ? 1 : 0);
        this.tmp_event.flip();
    ...
    private static boolean isKeyPressedAsync(int virt_key) {
        return (WindowsKeyboard.GetAsyncKeyState(virt_key) & 32768) != 0;
    }
    ...
    void releaseAll(long millis) {
        for (int i = 0; i < this.virt_key_down_buffer.length; ++i) {
            ...
            this.handleKey(i, 0, false, 0, millis, false);
        }
    }

    void handleKey(int virt_key, int scan_code, boolean extended, byte event_state, long millis, boolean repeat) {
        virt_key = WindowsKeyboard.translateExtended(virt_key, scan_code, extended);
        if (!repeat && WindowsKeyboard.isKeyPressed(event_state) == WindowsKeyboard.isKeyPressed(this.virt_key_down_buffer[virt_key])) {
            return;
        }
        this.flushRetained();
        ...
        if (keycode < this.key_down_buffer.length) {
            ...
            repeat &= WindowsKeyboard.isKeyPressed(this.virt_key_down_buffer[virt_key]);
    ...
    void handleChar(int event_char, long millis, boolean repeat) {
        ...
        if (!this.has_retained_event) {
            this.putEvent(0, 0, event_char, millis, repeat);
        } else {
