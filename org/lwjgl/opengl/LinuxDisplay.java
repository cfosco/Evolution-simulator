...
implements DisplayImplementation {
    ...
    private void grabPointer() {
        int result;
        if (!this.pointer_grabbed && (result = LinuxDisplay.nGrabPointer(LinuxDisplay.getDisplay(), LinuxDisplay.getWindow(), 0)) == 0) {
    ...
    public DisplayMode init() throws LWJGLException {
        ...
        try {
            ...
            switch (this.current_displaymode_extension) {
                case 10: {
                    this.saved_mode = (DisplayMode)AccessController.doPrivileged(new PrivilegedAction<DisplayMode>(){
        ...
        try {
            ...
            if (this.current_displaymode_extension == 10) {
                ...
                if (nDisplayModes.length > 0) {
                    bpp2 = nDisplayModes[0].getBitsPerPixel();
                }
                ...
                for (int i = 0; i < modes.length; ++i) {
                    modes[i] = new DisplayMode(resolutions[i].width, resolutions[i].height, bpp2, resolutions[i].freq);
                }
                DisplayMode[] i = modes;
                return i;
            }
            try {
                DisplayMode[] modes;
                bpp2 = modes = LinuxDisplay.nGetAvailableDisplayModes(LinuxDisplay.getDisplay(), LinuxDisplay.getDefaultScreen(), this.current_displaymode_extension);
            }
            catch (Throwable throwable) {
                LinuxDisplay.decDisplay();
                throw throwable;
            ...
            if (this.last_window_focus != current_focus_window || this.parent_focused != this.focused) {
                if (this.isParentWindowActive(current_focus_window)) {
                    if (this.parent_focused) {
                        LinuxDisplay.nSetInputFocus(LinuxDisplay.getDisplay(), current_window, 0);
                        this.last_window_focus = current_window;
                        this.focused = true;
                    } else {
                        LinuxDisplay.nSetInputFocus(LinuxDisplay.getDisplay(), this.parent_proxy_focus_window, 0);
    ...
    private void setInputFocusUnsafe(long window) {
        try {
            LinuxDisplay.nSetInputFocus(LinuxDisplay.getDisplay(), window, 0);
    ...
    private static final class Compiz {
                        ...
                        if (Compiz.isProcessActive("dbus-daemon")) {
                            ...
                            provider = new Provider(){
                                ...
                                public boolean hasLegacyFullscreenSupport() throws LWJGLException {
                                    List output = Compiz.run(new String[]{"dbus-send", "--print-reply", "--type=method_call", "--dest=org.freedesktop.compiz", "/org/freedesktop/compiz/workarounds/allscreens/legacy_fullscreen", "org.freedesktop.compiz.get"});
                                ...
                                public void setLegacyFullscreenSupport(boolean state) throws LWJGLException {
                                    if (Compiz.run(new String[]{"dbus-send", "--type=method_call", "--dest=org.freedesktop.compiz", "/org/freedesktop/compiz/workarounds/allscreens/legacy_fullscreen", "org.freedesktop.compiz.set", "boolean:" + Boolean.toString(state)}) == null) {
                            ...
                            try {
                                ...
                                provider = new Provider(){
                                    ...
                                    public boolean hasLegacyFullscreenSupport() throws LWJGLException {
                                        List output = Compiz.run(new String[]{"gconftool", "-g", "/apps/compiz/plugins/workarounds/allscreens/options/legacy_fullscreen"});
                                    ...
                                    public void setLegacyFullscreenSupport(boolean state) throws LWJGLException {
                                        if (Compiz.run(new String[]{"gconftool", "-s", "/apps/compiz/plugins/workarounds/allscreens/options/legacy_fullscreen", "-s", Boolean.toString(state), "-t", "bool"}) == null) {
                                            throw new LWJGLException("Failed to apply Compiz LFS workaround.");
                                        }
                                        if (state) {
                                            try {
        ...
        private static /* varargs */ List<String> run(String ... command) throws LWJGLException {
