...
final class LinuxMouse {
    ...
    LinuxMouse(long display, long window, long input_window) throws LWJGLException {
        ...
        this.button_count = LinuxMouse.nGetButtonCount(display);
    ...
    private void setCursorPos(boolean grab, int x, int y, long nanos) {
        ...
        if (dx != 0 || dy != 0) {
            ...
            this.last_y = y;
            if (grab) {
                this.putMouseEventWithCoords(-1, 0, dx, dy, 0, nanos);
            } else {
                this.putMouseEventWithCoords(-1, 0, x, y, 0, nanos);
            }
        }
    }
    ...
    private void doHandlePointerMotion(boolean grab, boolean warp_pointer, long root_window, int root_x, int root_y, int win_x, int win_y, long nanos) {
        ...
        boolean bl = outside_limits = root_x < border_left + 10 || root_y < border_top + 10 || root_x > border_right - 10 || root_y > border_bottom - 10;
        if (outside_limits) {
    ...
    private int transformY(int y) {
        return LinuxMouse.nGetWindowHeight(this.display, this.window) - 1 - y;
    }
    ...
    public void setCursorPosition(int x, int y) {
        LinuxMouse.nWarpCursor(this.display, this.window, x, this.transformY(y));
    }
    ...
    private void handlePointerMotion(boolean grab, boolean warp_pointer, long millis, long root_window, int x_root, int y_root, int x, int y) {
        this.doHandlePointerMotion(grab, warp_pointer, root_window, x_root, y_root, x, y, millis * 1000000);
    }
    ...
    private void handleButtonPress(boolean grab, byte button, long nanos) {
        ...
        switch (button) {
            case 4: {
                delta = 120;
                this.putMouseEvent(grab, -1, 0, delta, nanos);
                this.accum_dz += delta;
                break;
            }
            case 5: {
            ...
            default: {
                this.handleButton(grab, button, 1, nanos);
            }
        }
    }
    ...
    private void handleButtonEvent(boolean grab, long millis, int type, byte button) {
        ...
        switch (type) {
            case 5: {
                this.handleButton(grab, button, 0, nanos);
                break;
            }
            case 4: {
