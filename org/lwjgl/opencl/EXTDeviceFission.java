...
public final class EXTDeviceFission {
    ...
    public static int clRetainDeviceEXT(CLDevice device) {
        ...
        int __result = EXTDeviceFission.nclRetainDeviceEXT(device.getPointer(), function_pointer);
        if (__result == 0) {
            device.retain();
        }
        return __result;
    ...
    public static int clReleaseDeviceEXT(CLDevice device) {
        ...
        int __result = EXTDeviceFission.nclReleaseDeviceEXT(device.getPointer(), function_pointer);
        if (__result == 0) {
            device.release();
        }
        return __result;
    ...
    public static int clCreateSubDevicesEXT(CLDevice in_device, LongBuffer properties, PointerBuffer out_devices, IntBuffer num_devices) {
        ...
        if ((__result = EXTDeviceFission.nclCreateSubDevicesEXT(in_device.getPointer(), MemoryUtil.getAddress(properties), out_devices == null ? 0 : out_devices.remaining(), MemoryUtil.getAddressSafe(out_devices), MemoryUtil.getAddressSafe(num_devices), function_pointer)) == 0 && out_devices != null) {
