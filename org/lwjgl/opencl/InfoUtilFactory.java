...
final class InfoUtilFactory {
    ...
    implements CLMem.CLMemUtil {
        ...
        public ByteBuffer getInfoHostBuffer(CLMem mem) {
            long mem_flags;
            mem.checkValid();
            if (LWJGLUtil.DEBUG && ((mem_flags = this.getInfoLong(mem, 4353)) & 8) != 8) {
    ...
    implements CLKernel.CLKernelUtil {
        ...
        public void setArg(CLKernel kernel, int index, byte value) {
            CL10.clSetKernelArg(kernel, index, 1, APIUtil.getBufferByte(1).put(0, value));
        }

        @Override
        public void setArg(CLKernel kernel, int index, short value) {
            CL10.clSetKernelArg(kernel, index, 2, APIUtil.getBufferShort().put(0, value));
        }

        @Override
        public void setArg(CLKernel kernel, int index, int value) {
            CL10.clSetKernelArg(kernel, index, 4, APIUtil.getBufferInt().put(0, value));
        }

        @Override
        public void setArg(CLKernel kernel, int index, long value) {
            CL10.clSetKernelArg(kernel, index, 8, APIUtil.getBufferLong().put(0, value));
        }

        @Override
        public void setArg(CLKernel kernel, int index, float value) {
            CL10.clSetKernelArg(kernel, index, 4, APIUtil.getBufferFloat().put(0, value));
        }

        @Override
        public void setArg(CLKernel kernel, int index, double value) {
            CL10.clSetKernelArg(kernel, index, 8, APIUtil.getBufferDouble().put(0, value));
        }
    ...
    implements CLContext.CLContextUtil {
        ...
        public CLContext create(CLPlatform platform, List<CLDevice> devices, CLContextCallback pfn_notify, Drawable share_drawable, IntBuffer errcode_ret) throws LWJGLException {
            ...
            properties.put(4228).put(platform);
            ...
            try {
                __result = new CLContext(CL10.nclCreateContext(MemoryUtil.getAddress0(properties.getBuffer()), devices.size(), MemoryUtil.getAddress(properties, propertyCount), pfn_notify == null ? 0 : pfn_notify.getPointer(), user_data, MemoryUtil.getAddressSafe(errcode_ret), function_pointer), platform);
                if (LWJGLUtil.DEBUG) {
                    Util.checkCLError(errcode_ret.get(0));
                }
