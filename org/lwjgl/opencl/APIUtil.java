...
final class APIUtil {
    ...
    static long getPointerSafe(PointerWrapper pointer) {
        return MemoryUtil.getAddress0(APIUtil.getBufferPointer().put(0, pointer == null ? 0 : pointer.getPointer()));
    }
    ...
    static CLPlatform getCLPlatform(PointerBuffer properties) {
        ...
        for (int k = 0; k < keys && (key = properties.get(k << 1)) != 0; ++k) {
            if (key != 4228) continue;
