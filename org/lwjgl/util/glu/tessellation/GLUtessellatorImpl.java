...
implements GLUtessellator {
    ...
    public void gluTessProperty(int which, double value) {
        switch (which) {
            ...
            case 100140: {
                ...
                if ((double)windingRule != value) break;
                switch (windingRule) {
                    case 100130: 
                    case 100131: 
                    case 100132: 
    ...
    private boolean flushCache() {
        ...
        for (int i = 0; i < this.cacheCount; ++i) {
            ...
            if (this.addVertex(vertex.coords, vertex.data)) continue;
            return false;
        }
        this.cacheCount = 0;
    ...
    public void gluTessEndPolygon() {
        try {
            ...
            if (!this.fatalError) {
                boolean rc = true;
                rc = this.boundaryOnly ? TessMono.__gl_meshSetWindingNumber(mesh, 1, true) : TessMono.__gl_meshTessellateInterior(mesh);
                if (!rc) {
                    throw new RuntimeException();
                }
