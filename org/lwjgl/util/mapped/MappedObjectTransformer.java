...
public class MappedObjectTransformer {
    ...
    private static FieldInfo registerField(boolean autoGenerateOffsets, String className, long advancingOffset, Field field) {
        ...
        if (field.getType() == Long.TYPE || field.getType() == Double.TYPE) {
            byteLength = pointer == null ? 8 : (long)MappedObjectUnsafe.INSTANCE.addressSize();
        ...
        if (pad != null) {
            ...
            assert (!pad.before() || byteOffset % (long)CacheUtil.getCacheLineSize() == 0);
            assert (!pad.after() || (byteOffset + byteLengthPadded) % (long)CacheUtil.getCacheLineSize() == 0);
        }
        if (PRINT_ACTIVITY) {
        ...
        switch (methodInsn.getOpcode()) {
            case 182: {
                if ("asArray".equals(methodInsn.name) && methodInsn.desc.equals("()[L" + MAPPED_OBJECT_JVM + ";")) {
                    ...
                    Frame<BasicValue> frame = frameMap.get((Object)nextInstruction);
    ...
    static int transformArrayAccess(InsnList instructions, int i, Map<AbstractInsnNode, Frame<BasicValue>> frameMap, VarInsnNode loadInsn, MappedSubtypeInfo mappedSubtype, int var) {
        block8 : {
            loadStackSize = frameMap.get((Object)loadInsn).getStackSize() + 1;
            nextInsn = loadInsn;
            do lbl-1000: // 3 sources:
            {
                ...
                frame = frameMap.get((Object)nextInsn);
                if (frame == null) ** GOTO lbl-1000
                stackSize = frame.getStackSize();
                if (stackSize == loadStackSize + 1 && nextInsn.getOpcode() == 50) {
                    aaLoadInsn = nextInsn;
                    break block8;
                }
                if (stackSize != loadStackSize || nextInsn.getOpcode() != 190) continue;
        ...
        while ((nextInsn = nextInsn.getNext()) != null) {
            ...
            if (stackSize == loadStackSize && nextInsn.getOpcode() == 89 && nextInsn.getNext().getOpcode() == 180) {
                ...
                getter = new MethodInsnNode(184, mappedSubtype.className, MappedObjectTransformer.getterName(fieldInsn.name), "(L" + mappedSubtype.className + ";I)" + fieldInsn.desc);
    ...
    private static /* varargs */ void getClassEnums(Class clazz, Map<Integer, String> map, String ... prefixFilters) {
        try {
    ...
    static String getOpcodeName(AbstractInsnNode insn) {
        String op;
        return INSNTYPE_TO_NAME.get(insn.getType()) + ": " + insn.getOpcode() + ((op = OPCODE_TO_NAME.get(insn.getOpcode())) == null ? "" : new StringBuilder().append(" [").append(OPCODE_TO_NAME.get(insn.getOpcode())).append("]").toString());
    }
    ...
    extends ClassAdapter {
                ...
                return new FieldNode(access, name, desc, signature, value){
                    ...
                    public void visitEnd() {
                        ...
                        for (AnnotationNode pad : this.visibleAnnotations) {
                            if (!MappedObjectTransformer.CACHE_LINE_PAD_JVM.equals(pad.desc)) continue;
        ...
        public MethodVisitor visitMethod(int access, String name, String desc, String signature, String[] exceptions) {
            ...
            if ("<init>".equals(name) && (mappedSubtype = MappedObjectTransformer.className_to_subtype.get(this.className)) != null) {
                if (!"()V".equals(desc)) {
